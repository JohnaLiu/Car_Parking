...
class RegisterNewCar extends JFrame {
		...
		public RegisterNewCar() {
			...
			lblRegisterNewCar.setFont(new Font("Times New Roman", Font.PLAIN, 28));
		...
		if( registerInput.equals("There is no such a ID number."))
			{
			JOptionPane.showMessageDialog(null, "You can't register since you are not a QM staff.", "FAILURE", JOptionPane.ERROR_MESSAGE);
			}
		else{
			try{
				...
				if (line.equals(registerInput))
					{
					JOptionPane.showMessageDialog(null, "You can't register again since you have registered", "FAILURE", JOptionPane.ERROR_MESSAGE);
						flag=1;
						break;
					}
					line=br.readLine();
			...
			JOptionPane.showMessageDialog(null, "Congratuation! You are registered in the car park successfully!", "SUCCESS", JOptionPane.INFORMATION_MESSAGE);
			}
			}
...
class ModifyParkInfo extends JFrame {
		...
		public ModifyParkInfo() {
			...
			lblModifyParkInfo.setFont(new Font("Times New Roman", Font.PLAIN, 28));
